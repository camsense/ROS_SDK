...
bool HCLidar::setLidarPara(const char* chLidarModel)
{
   ...
   if (m_strLidarModel == T1A)
	...
	{
	   m_sAttr.dAngleOffsetD = 21; //28.5;
	   m_sAttr.dBaseline_mm = 20; //17.92;
	   m_sAttr.iFPSMax = FPS_TOF_MAX; //FPS_2000_MAX;
	   m_sAttr.iFPSMin = FPS_TOF_MIN; // FPS_2000_MIN;
	   m_sAttr.iSpeedMax = SPEED_TOF_MAX; //SPEED_312_MAX;
	   m_sAttr.iSpeedMin = SPEED_TOF_MIN; //SPEED_312_MIN;
	   m_sAttr.dAngleStep = ANGLE_RESOLV_TOF; //ANGLE_RESOLV_2000;
	   m_sAttr.dCirclePoints = CICRLE_MAX_TOF;//CICRLE_MAX_2000;
	}
    else
    {
...
bool HCLidar::parserRangeEX(LstPointCloud &resultRange, const char * chBuff, int iIndex, int in_numData, int iPointSize)
{
	...
	UINT64 epochTime = HCHead::getCurrentTimestampUs() / 1000;
...
bool HCLidar::getRxPointClouds(LstPointCloud& lstG)
{
	...
	if (m_bCircle)
	{
		if (m_Circles.size() > 0)
		{
			//LOG_INFO("HCSDK Info: getRxPointClouds\n", m_lstBuff.size());
